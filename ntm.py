...
class NTMCell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, controller_layers, controller_units, memory_size, memory_vector_dim, read_head_num, write_head_num,
                 addressing_mode, shift_range, reuse, output_dim, clip_value, init_mode):
        ...
        def single_cell(num_units):
            return tf.nn.rnn_cell.LSTMCell(num_units, forget_bias=1.0)

        self.controller = tf.nn.rnn_cell.MultiRNNCell([single_cell(self.controller_units) for _ in range(self.controller_layers)])

...
class NTMCell(keras.layers.AbstractRNNCell):
    ...
    def call(self, x, prev_state):
        ...
        controller_input = tf.keras.layers.Concatenate(axis=1)([x] + prev_read_vector_list)
        ...
        NTM_output = self.output_proj(tf.keras.layers.Concatenate(axis=1)([controller_output] + read_vector_list))
    ...
    def _addressing(self, k, beta, g, s, gamma, prev_M, prev_w):
        ...
        s = tf.keras.layers.Concatenate(axis=1)([s[:, :self.shift_range + 1],
                       tf.zeros([s.get_shape()[0], self.memory_size - (self.shift_range * 2 + 1)]),
                       s[:, -self.shift_range:]])
        t = tf.keras.layers.Concatenate(axis=1)([tf.reverse(s, axis=[1]), tf.reverse(s, axis=[1])])
        s_matrix = tf.stack(
